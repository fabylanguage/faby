...
/* Faby tooling — fmt (canonical formatter) and check (static analysis).
   Pure functions, work in the browser and in Node. Depend on the
   interpreter's parseProgram / builtinNames (globals or require). */
(function (global) {
'use strict';

const parseProgram = global.fabyParseProgram ||
  (typeof require !== 'undefined' ? require('./faby-interpreter.js').parseProgram : null);
const builtinNames = global.fabyBuiltinNames ||
  (typeof require !== 'undefined' ? require('./faby-interpreter.js').builtinNames : () => []);

/* ============================================================
   faby fmt — canonical formatter
   Rules (idempotent):
   - 2-space indentation per nesting level
   - collapse runs of internal spaces to one (outside strings) → no manual alignment
   - exactly one space after a comma, none before
   - single space around -> and |>
   - strip trailing whitespace
   - collapse 3+ blank lines; one blank line between top-level blocks
...
function normalizeSpacing(code) {
  ...
  // arrows / pipe: one space around
  protectedCode = protectedCode.replace(/\s*->\s*/g, ' -> ');
  protectedCode = protectedCode.replace(/\s*\|>\s*/g, ' |> ');
  protectedCode = protectedCode.trimEnd();
  // restore strings
  protectedCode = protectedCode.replace(/ (\d+) /g, (_, i) => strs[+i]);
  return protectedCode;
}

function fmtFaby(src) {
  const raw = src.replace(/\r\n/g, '\n').split('\n');

  const out = [];
  let blanks = 0;
  const stack = [0];   // indentation columns → nesting depth (off-side rule)
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    code = normalizeSpacing(code.trim());
    comment = comment.trim();
    const isFullComment = (code === '' && comment !== '');

    // derive nesting depth from relative indentation, not a guessed unit
    let depth;
    if (isFullComment) {
      depth = stack.length - 1;            // comments don't change nesting
    } else {
    ...
    let rebuilt;
    if (code === '' && comment) {
      rebuilt = '  '.repeat(depth) + comment;          // full-line comment
    } else if (comment) {
      rebuilt = '  '.repeat(depth) + code + '  ' + comment; // inline comment, 2 spaces before
    } else {
      rebuilt = '  '.repeat(depth) + code;
    }

    // blank-line policy: collapse to at most 1; keep a single blank between
    // top-level (depth 0) blocks; drop leading blanks
...
/* ============================================================
   faby check — static analysis (no execution)
   Diagnostics: unknown names, flow arity, non-exhaustive match,
   immutable reassignment, duplicate flow definitions.
   ============================================================ */

function collectFlows(stmts, flows) {
  for (const s of stmts) {
    if (s.k === 'flowdef') {
      flows[s.name] = { arity: s.params.length, line: s.line, dup: s.name in flows };
      collectFlows(s.body, flows);
    } else if (s.body) collectFlows(s.body, flows);
  }
}

function checkFaby(src) {
  const diags = [];
  let stmts;
  try {
    stmts = parseProgram(src);
  } catch (e) {
    const d = (e && e.faby)
      ? { sev: 'error', code: 'parse', msg: e.message, line: e.fabyLine || 1 }
      : { sev: 'error', code: 'internal', msg: e.message, line: 1 };
    return { ok: false, errors: 1, warns: 0, flows: 0, diagnostics: [d] };
  }

  const builtins = new Set(builtinNames());
  const flows = {};
  collectFlows(stmts, flows);
  for (const name in flows) {
    if (flows[name].dup) diags.push({ sev: 'error', code: 'dup-flow', msg: 'flow "' + name + '" is defined more than once', line: flows[name].line });
  }
  const flowSet = new Set(Object.keys(flows));
  const known = new Set([...builtins, ...flowSet, 'result', 'self', '_', 'true', 'false', 'none', 'and', 'or', 'not']);
  ...
  function walkExpr(e, scope) {
    ...
    switch (e.k) {
      case 'id':
        if (!scope.has(e.name) && !known.has(e.name) && !/^[A-Z]/.test(e.name))
          diags.push({ sev: 'error', code: 'unknown-name', msg: 'unknown name "' + e.name + '"', line: e.line });
        break;
      case 'call':
        // arity check for direct calls to user flows
        if (e.callee && e.callee.k === 'id' && flows[e.callee.name]) {
          const want = flows[e.callee.name].arity, got = e.args.length;
          if (got !== want)
            diags.push({ sev: 'warn', code: 'arity', msg: 'flow "' + e.callee.name + '" expects ' + want + ' argument' + (want === 1 ? '' : 's') + ', got ' + got + (got < want ? ' (a piped value counts as one)' : ''), line: e.line });
        }
        walkExpr(e.callee, scope);
        e.args.forEach(a => walkExpr(a, scope));
        break;
      case 'lambda': { const s2 = new Set(scope); s2.add('_'); walkExpr(e.body, s2); break; }
      case 'bin': walkExpr(e.l, scope); walkExpr(e.r, scope); break;
      case 'neg': case 'not': walkExpr(e.e, scope); break;
      case 'member': walkExpr(e.obj, scope); break;
      case 'list': e.items.forEach(x => walkExpr(x, scope)); break;
      case 'record': e.pairs.forEach(p => walkExpr(p.val, scope)); break;
      case 'await': walkExpr(e.e, scope); break;
      case 'recover': walkExpr(e.expr, scope); if (e.arg) walkExpr(e.arg, scope); break;
      // string interpolations ({…}) are parsed lazily at runtime; skip to avoid false positives
    }
  }
    ...
    for (const s of stmts) {
      switch (s.k) {
        case 'flowdef': {
          ...
          s.params.forEach(p => s2.add(p));
          walk(s.body, s2);
          break;
        }
        case 'decl': walkExpr(s.expr, scope); scope.add(s.name); s._mut = s.mut; break;
        case 'assign': walkExpr(s.expr, scope);
          if (!scope.has(s.name) && !known.has(s.name))
            diags.push({ sev: 'error', code: 'unknown-name', msg: 'unknown name "' + s.name + '"', line: s.line });
          break;
        case 'aug': walkExpr(s.expr, scope);
          if (!scope.has(s.name) && !known.has(s.name))
            diags.push({ sev: 'error', code: 'unknown-name', msg: 'unknown name "' + s.name + '"', line: s.line });
          break;
        case 'contract': walkExpr(s.expr, scope); break;
        case 'if': walkExpr(s.cond, scope); walk(s.body, new Set(scope)); if (s.elseBody) walk(s.elseBody, new Set(scope)); break;
        case 'for': { walkExpr(s.expr, scope); const s2 = new Set(scope); s2.add(s.var); walk(s.body, s2); break; }
        case 'match': {
          walkExpr(s.subject, scope);
          let hasCatchAll = false;
          for (const arm of s.arms) {
            for (const alt of arm.alts) { if (alt.any) hasCatchAll = true; else walkExpr(alt.expr, scope); }
            if (arm.guard) walkExpr(arm.guard, scope);
            walkExpr(arm.result, scope);
          }
          if (!hasCatchAll)
            diags.push({ sev: 'warn', code: 'match-exhaustive', msg: 'match may not be exhaustive — add a "_" arm to be safe', line: s.line });
          break;
        }
        case 'expr': walkExpr(s.expr, scope); break;
      }
    }
  }
  ...
  return { ok: errors === 0, errors, warns, flows: flowSet.size, diagnostics: diags };
}

/* ============================================================
   faby test — test runner with contract-derived property fuzzing
   - runs `test "..."` blocks (pass = no error)
   - for every flow with contracts, generates random typed inputs,
     skips ones that fail `expect` (precondition), and flags any input
     that passes `expect` but violates `ensure` or crashes (counterexample)
   ============================================================ */

const runFaby = global.runFaby ||
  (typeof require !== 'undefined' ? require('./faby-interpreter.js').runFaby : null);

function _rand() { return Math.random(); }

function genArg(type) {
  let base = (type || 'Int').trim().replace(/\s+where[\s\S]*$/i, '').trim();
  if (base[0] === '[') {
    const inner = base.slice(1, -1).trim() || 'Int';
    const n = Math.floor(_rand() * 4);
    return '[' + Array.from({ length: n }, () => genArg(inner)).join(', ') + ']';
  }
  switch (base) {
    case 'Int': return String(Math.floor(_rand() * 100) - 50);
    case 'Float': return (Math.round((_rand() * 100 - 50) * 100) / 100).toFixed(2);
    case 'Bool': return _rand() < 0.5 ? 'true' : 'false';
    case 'Text': { const p = ['faby', 'rust', '', 'hello world', 'x', 'AaBb', '42']; return JSON.stringify(p[Math.floor(_rand() * p.length)]); }
    case 'Email': return '"ada@faby.dev"';
    case 'Url': return '"https://faby.codes"';
    default: return String(Math.floor(_rand() * 100) - 50);   // unknown type → small int
  }
}
...
function extractTests(src) {
  ...
  while (i < lines.length) {
    const m = lines[i].match(/^test\s+"((?:[^"\\]|\\.)*)"\s*$/);
    if (m) {
      const name = m[1], body = []; i++;
      while (i < lines.length && (lines[i].trim() === '' || /^\s/.test(lines[i]))) { body.push(lines[i]); i++; }
      tests.push({ name, body: body.join('\n') });
    } else { kept.push(lines[i]); i++; }
  }
  return { stripped: kept.join('\n'), tests };
}

function testFaby(src, rounds) {
  rounds = rounds || 100;
  if (!runFaby) return { ok: false, error: 'interpreter unavailable' };
  let stmts;
  try { stmts = parseProgram(src); }
  catch (e) { return { ok: false, results: [{ kind: 'parse', name: '(parse)', pass: false, msg: (e && e.message) || String(e), line: e && e.fabyLine }] }; }

  const { stripped, tests } = extractTests(src);
  const results = [];

  // 1) explicit test blocks
  for (const t of tests) {
    const prog = stripped + '\nflow main\n' + (t.body || '  print("ok")');
    const r = runFaby(prog);
    results.push({ kind: 'test', name: t.name, pass: !r.error, msg: r.error || '' });
  }

  // 2) contract-derived property tests
  function collect(stmts) {
    let flows = [];
    for (const s of stmts) {
      if (s.k === 'flowdef') {
        const hasContract = s.body.some(b => b.k === 'contract');
        if (hasContract && s.params.length > 0 && s.name !== 'main')
          flows.push(s);
        flows = flows.concat(collect(s.body));
      }
    }
    return flows;
  }
  for (const f of collect(stmts)) {
    let passed = 0, skipped = 0, counter = null;
    for (let i = 0; i < rounds; i++) {
      const args = f.params.map((_, idx) => genArg(f.ptypes && f.ptypes[idx]));
      const prog = stripped + '\nflow main\n  let __probe = ' + f.name + '(' + args.join(', ') + ')\n';
      const r = runFaby(prog);
      if (!r.error) { passed++; continue; }
      if (/contract violated: expect/.test(r.error)) { skipped++; continue; }
      counter = { args, error: r.error.replace(/ \(line \d+\)$/, '') };
      break;
    }
    results.push({
      kind: 'property', name: f.name, pass: !counter,
      passed, skipped, rounds,
      msg: counter ? 'counterexample ' + f.name + '(' + counter.args.join(', ') + ') → ' + counter.error : ''
    });
  }
...
global.fmtFaby = fmtFaby;
global.checkFaby = checkFaby;
global.testFaby = testFaby;
if (typeof module !== 'undefined' && module.exports) module.exports = { fmtFaby, checkFaby, testFaby };
})(typeof window !== 'undefined' ? window : globalThis);
...
const runFaby = globalThis.runFaby;
const fmtFaby = globalThis.fmtFaby;
const checkFaby = globalThis.checkFaby;
const testFaby = globalThis.testFaby;
